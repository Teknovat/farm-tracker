...
export function validateRequestBody<T>(schema: ZodSchema<T>, body: any): ValidationResult<T> {
  try {
...
export const animalCreateSchema = z
  .object({
    ...
    species: z
      ...
      .max(100, "Species name cannot exceed 100 characters")
      .trim(),
    sex: z
      .enum(["MALE", "FEMALE"], {
    ...
    lotCount: z
      ...
      .int("Lot count must be a whole number")
      .positive("Lot count must be positive")
      .max(10000, "Lot count cannot exceed 10,000")
      .optional(),
  })
  .refine(
    (data) => {
