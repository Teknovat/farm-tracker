...
export async function createDepositSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        amount: z.number({
            ...
            errorMap
        }).positive(t('amount.positive')).max(1000000, t('amount.maxExceeded', { max: '1,000,000' })),
        description: z.string({
            required_error: t('description.required'),
            errorMap
        }).min(1, t('description.empty')).max(255, t('description.maxLength', { max: 255 })).trim()
    })
}
...
export async function createReimbursementSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        ...
        amount: z.number({
            ...
            errorMap
        }).positive(t('reimbursementAmount.positive')).max(1000000, t('reimbursementAmount.maxExceeded', { max: '1,000,000' })),
        description: z.string({ errorMap }).max(255, t('description.maxLength', { max: 255 })).trim().optional()
    })
}
...
export async function createAnimalCreateSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        ...
        species: z.string({
            ...
            errorMap
        }).min(1, t('species.empty')).max(100, t('species.maxLength', { max: 100 })).trim(),
        ...
        estimatedAge: z.number({
            ...
            errorMap
        }).int(t('estimatedAge.int')).min(0, t('estimatedAge.negative')).max(50, t('estimatedAge.maxExceeded', { max: 50 })).optional(),
        ...
        lotCount: z.number({
            ...
            errorMap
        }).int(t('lotCount.int')).positive(t('lotCount.positive')).max(10000, t('lotCount.maxExceeded', { max: 10000 })).optional()
    }).refine(data => {
...
export async function createAnimalUpdateSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        species: z.string({ errorMap }).min(1, t('species.empty')).max(100, t('species.maxLength', { max: 100 })).trim().optional(),
        sex: z.enum(['MALE', 'FEMALE'], {
            invalid_type_error: t('sex.invalid'),
            errorMap
        }).optional(),
...
export async function createEventCreateSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        ...
        note: z.string({ errorMap }).max(1000, t('note.maxLength', { max: 1000 })).trim().optional(),
        cost: z.number({
            invalid_type_error: t('cost.invalidType'),
            errorMap
        }).min(0, t('cost.negative')).max(1000000, t('cost.maxExceeded', { max: '1,000,000' })).optional(),
...
export async function createFarmCreateSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        name: z.string({
            ...
            errorMap
        }).min(1, t('farmName.empty')).max(100, t('farmName.maxLength', { max: 100 })).trim(),
...
export async function createFarmUpdateSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        name: z.string({ errorMap }).min(1, t('farmName.empty')).max(100, t('farmName.maxLength', { max: 100 })).trim().optional(),
...
export async function createMemberInviteSchema(locale: Locale = 'fr') {
    ...
    return z.object({
        email: z.string({
            ...
            errorMap
        }).email(t('email.invalid')).toLowerCase(),
